...
for i in range(1, Tests_num + 1):
    ...
    f1 = open('./P2_testcases/T' + ('0' + str(i) if i < 10 else str(i)) + '/parse_tree.txt', 'r')
    f2 = open('./P2_testcases/T' + ('0' + str(i) if i < 10 else str(i)) + '/parse_tree_result.txt', 'r')
    print("Parse Tree:")
    ...
    f1.close()
    f2.close()
    f1 = open('./P2_testcases/T' + ('0' + str(i) if i < 10 else str(i)) + '/syntax_errors.txt', 'r')
    f2 = open('./P2_testcases/T' + ('0' + str(i) if i < 10 else str(i)) + '/syntax_errors_result.txt', 'r')
    print("Syntax errors:")
    if re.sub(r'\s+', '', f1.read()) == re.sub(r'\s+', '', f2.read()):
        print("Passed")
    else:
